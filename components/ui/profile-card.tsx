...
export const ProfileCard = memo(function ProfileCard({ profile, onPress }: ProfileCardProps) {
  ...
  return (
    ...
    >
      ...
      <View style={styles.content}>
        <View style={styles.header}>
          ...
          {displayLocation && (
            <View style={styles.locationRow}>
              ...
              <ThemedText style={[styles.location, { color: theme.icon }]} numberOfLines={1}>
                {displayLocation}
              </ThemedText>
            </View>
          )}
        ...
        {displayBio && (
          <ThemedText style={[styles.bio, { color: theme.icon }]} numberOfLines={2}>
            {displayBio}
          </ThemedText>
        )}
        ...
        {interestsArray.length > 0 && (
          <View style={styles.interestsRow}>
            {interestsArray.map((interest, index) => (
              <View
                key={index}
                style={[styles.interestBadge, { backgroundColor: theme.primary + '15', borderColor: theme.primary + '30' }]}
              >
      ...
      {/* Chevron */}
      <MaterialIcons name="chevron-right" size={24} color={theme.icon} style={styles.chevron} />
    </Pressable>
  );
});
